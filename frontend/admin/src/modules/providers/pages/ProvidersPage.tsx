/** 数据源页，展示 provider 列表、健康状态和备注信息。 */
import { useEffect, useState } from 'react'
import type { ProviderStatus } from '../../../common/appTypes'
import { DetailPill, EmptyPanel } from '../../../common/components/OpsUi'
...
export function ProvidersPage({ providers }: { providers: ProviderStatus[] }) {
  ...
  const selectedProvider = providers.find((provider) => provider.providerKey === selectedProviderKey) ?? providers[0] ?? null
          ...
          {providers.map((provider) => (
            <button
              ...
              className={`w-full rounded-[1.6rem] border p-4 text-left transition ${
                selectedProvider?.providerKey === provider.providerKey ? 'border-[color:var(--fc-color-accent)]/45 bg-[color:var(--fc-color-accent)]/8' : 'border-white/8 bg-slate-950/35 hover:border-white/20'
              }`}
            >
              <div className="flex items-center justify-between gap-4">
                <p className="font-medium text-white">{provider.providerKey}</p>
                <span className="rounded-full bg-emerald-400/10 px-3 py-1 text-xs uppercase tracking-[0.18em] text-emerald-300">{provider.status}</span>
              </div>
