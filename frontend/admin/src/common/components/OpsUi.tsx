/** 后台通用 UI 小组件，集中放置状态徽标、空态、指标卡和登录壳层。 */
  ...
  return (
    <span
      className={`rounded-full px-3 py-1 text-xs uppercase tracking-[0.18em] ${
        flag.enabled ? 'bg-emerald-400/10 text-emerald-300' : 'bg-slate-500/20 text-slate-300'
      }`}
    >
      {flag.environment}
    </span>
...
export function LoginShell({ children }: PropsWithChildren) {
  return <div className="mx-auto flex min-h-screen max-w-6xl items-center px-6 py-8">{children}</div>
}
