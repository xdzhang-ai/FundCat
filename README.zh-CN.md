...
如果你在 `.env` 里修改了端口或账号密码，启动 API 前请同步导出对应的 `DB_*` 变量。
...
- 主站构建：`cd apps/web && npm_config_cache=../../.npm-cache npm run build`
- 后台构建：`cd apps/admin && npm_config_cache=../../.npm-cache npm run build`
- API 测试：`cd services/api && export JAVA_HOME=$(/usr/libexec/java_home -v 21) && mvn test`
